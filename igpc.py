...
def counterfact_loss(
    ...
    for h in range(H):
        ...
        if w_is == "de":
            y = y.unflatten(env_sim(y, u).flatten() + W[h + M])
        elif w_is == "dede":
            y = y.unflatten(env_sim(y, u).flatten() + D[h + M] + W[h + M]) 
        else:
            y = y.unflatten(
                X_old[h + M + 1].flatten()
...
def GPC_rollout(
    env_true, env_sim, cost_func, U_old, k, K, X_old, D, F, alpha, w_is, ref_lr=0.1
):
    # print(ref_lr)
    ...
    for h in range(env_true.H):
        ...
        U[h] = U_old[h] + alpha * k[h] + K[h] @ (X[h].flatten() - X_old[h].flatten()) + C * U_delta[h]
        X[h + 1] = env_true(X[h], U[h])
        cost += cost_func(X[h], U[h], env_true)

        # 3 choices for w are f-g, (f-g)-(f-g last time), (f- LIN(g) with OFF).
        if w_is == "de":
            w = X[h + 1].flatten() - env_sim(X[h], U[h]).flatten()
        elif w_is == "dede":
            w = (X[h + 1].flatten() - env_sim(X[h], U[h]).flatten()) - D[h]
        else:
            w = X[h + 1].flatten() - (
                X_old[h + 1].flatten() + F[h][0] @ (X[h].flatten() - X_old[h].flatten()) + F[h][1] @ (U[h] - U_old[h])
            )
...
def iGPC_closed(
    env_true, env_sim, cost_func, U, T, k=None, K=None, X=None, w_is="de", 
    lr=None, ref_alpha=1.0, backtracking=True, verbose=True
):
    alpha, r = ref_alpha, 1
    ...
    for t in range(T):
        ...
        if backtracking:
            ...
            r += 1
            X, U, c = GPC_rollout(
                env_true, env_sim, cost_func, U, k, K, X, D, F, alpha, w_is, lr
            )
            if verbose:
                print(
                    f"iGPC: t = {t}, r = {r}, c = {c}"
