...
def iLQR(env, cost, U, T, k=None, K=None, X=None, info="true", start_state=None, 
         H=None, render=False, render_dir=".", verbose=True, alpha=0.5, backtracking=True):
    r = 1
    if H == None:
        H = env.H
    X, U, c = rollout(env, cost, U, k, K, X, start_state=start_state, H=H, 
                      render=render, render_dir=os.path.join(render_dir, "t=0_r=1"))
    if verbose:
    ...
    for t in range(T):
        ...
        if backtracking:
            for alphaC in alpha * 1.1 * 1.1 ** (-jnp.arange(10) ** 2):
                r += 1
                #s = time.time()
                XC, UC, cC = rollout(env, cost, U, k, K, X, alpha, start_state=start_state, H=H, 
                    render=render,render_dir=os.path.join(render_dir, f"t={t}_r={r}"))
                if verbose:
            ...
            r += 1
            X, U, c = rollout(env, cost, U, k, K, X, alpha, start_state=start_state, H=H, 
                render=render,render_dir=os.path.join(render_dir, f"t={t}_r={r}"))
            if verbose:
